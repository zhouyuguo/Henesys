...
class Parser:
    ...
    def dump(self, xml_dicts, file_id_str, day_stamp = datetime.date.today()):
        content_str = str() 
    ...
    def run(self, day_stamp = datetime.date.today()):
        _dir = os.path.join(global_define.XML_DIR, str(day_stamp))
        file_paths = functs.get_files(_dir)
        for i,file_path in enumerate(file_paths):
            xml_dicts = self.parse(file_path)
            if xml_dicts:
                self.dump(xml_dicts, str(i), day_stamp)
        pass
