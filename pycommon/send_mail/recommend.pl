...
foreach my $ch (@cloud_recommend_channel)
{
	...
	else
	{
		...
		if( $VENDOR_ID == 100500  and $ch eq "recommend")
		{
			$h_ch{$ch}{"pv"} = @result[0] + $h_ch{"wpr_refer"}{"pv"};
			$h_ch{$ch}{"click"} = @result[1] +  $h_ch{"wpr_refer"}{"click"};
